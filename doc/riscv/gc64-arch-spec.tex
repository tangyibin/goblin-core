...
\author{John D. \textsc{Leidel}, Xi \textsc{Wang}, Yong \textsc{Chen}}
...
\begin{center}
\begin{tabular}{l r}
Version: & 0.3PRERELEASE \\ % Date the experiment was performed
...
to the largest (most encompassing) is depicted in the list below.
Note that each subsequent layer encompasses one or more units from
the previous layer.  The minimum GC64 configuration requires layers 0-3.  

\begin{center}
\begin{enumerate}
\item[0.] Task Unit [required]
\item[1.] Task Processor [required]
\item[2.] Task Group [required]
\item[3.] Socket [required]
\item[4.] Node [optional]
\item[5.] Partition [optional]
\end{enumerate}
\end{center}
...
cycle.  In this manner, each task processor permits full control from a single task unit 
at any given time.  It is the job of the thread control
...
\makebox[2.0in][s]{127 96}\makebox[0.03in][s]{}\makebox[1.0in][s]{95 80}\makebox[0.03in][s]{}\makebox[1.0in][s]{79 64}
