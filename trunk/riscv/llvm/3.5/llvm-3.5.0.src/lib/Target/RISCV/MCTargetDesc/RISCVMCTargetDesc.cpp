...
static MCAsmInfo *createRISCVMCAsmInfo(const MCRegisterInfo &MRI, StringRef TT) {
  ...
  unsigned SP = MRI.getDwarfRegNum(RISCV::sp, true);
...
static MCStreamer *createRISCVMCObjectStreamer(const Target &T, StringRef TT,
  ...
  return createELFStreamer(Context, MAB, OS, Emitter, RelaxAll, NoExecStack);
}
