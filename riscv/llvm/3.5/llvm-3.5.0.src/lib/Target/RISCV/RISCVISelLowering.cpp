...
EVT RISCVTargetLowering::getSetCCResultType(LLVMContext &, EVT VT) const {
	return MVT::i32;
}
...
RCPair
RISCVTargetLowering::getRegForInlineAsmConstraint(const std::string &Constraint,
                                                MVT VT) const {
  if (Constraint.size() == 1) {
...
static SDValue convertLocVTToValVT(SelectionDAG &DAG, DebugLoc DL,
  ...
  SDLoc SL( Value.getNode() );
  if (VA.getLocInfo() == CCValAssign::SExt)
    Value = DAG.getNode(ISD::AssertSext, SL, VA.getLocVT(), Value,
                        DAG.getValueType(VA.getValVT()));
  else if (VA.getLocInfo() == CCValAssign::ZExt)
    Value = DAG.getNode(ISD::AssertZext, SL, VA.getLocVT(), Value,
                        DAG.getValueType(VA.getValVT()));

  if (VA.isExtInLoc())
    Value = DAG.getNode(ISD::TRUNCATE, SL, VA.getValVT(), Value);
  else if (VA.getLocInfo() == CCValAssign::Indirect)
    Value = DAG.getLoad(VA.getValVT(), SL, Chain, Value,
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDLoc DL,
  ...
  case CCValAssign::SExt:
    return DAG.getNode(ISD::SIGN_EXTEND, SL, VA.getLocVT(), Value);
  case CCValAssign::ZExt:
    return DAG.getNode(ISD::ZERO_EXTEND, SL, VA.getLocVT(), Value);
  case CCValAssign::AExt:
    return DAG.getNode(ISD::ANY_EXTEND, SL, VA.getLocVT(), Value);
  case CCValAssign::BCvt:
    return DAG.getNode(ISD::BITCAST, SL, VA.getLocVT(), Value);
...
copyByValRegs(SDValue Chain, DebugLoc DL, std::vector<SDValue> &OutChains,
  ...
  SDLoc SL( Chain.getNode() );
  for (unsigned I = 0; I < ByVal.NumRegs; ++I) {
    unsigned ArgReg = CC.intArgRegs()[ByVal.FirstIdx + I];
    unsigned VReg = addLiveIn(MF, ArgReg, RC);
    unsigned Offset = I * CC.regSize();
    SDValue StorePtr = DAG.getNode(ISD::ADD, SL, PtrTy, FIN,
                                   DAG.getConstant(Offset, PtrTy));
    SDValue Store = DAG.getStore(Chain, SL, DAG.getRegister(VReg, RegTy),
...
passByValArg(SDValue Chain, DebugLoc DL,
  ...
  if (ByVal.NumRegs) {
    ...
    for (; I < ByVal.NumRegs - LeftoverBytes; ++I, Offset += RegSize) {
      SDValue LoadPtr = DAG.getNode(ISD::ADD, SL, PtrTy, Arg,
                                    DAG.getConstant(Offset, PtrTy));
      SDValue LoadVal = DAG.getLoad(RegTy, SL, Chain, LoadPtr,
    ...
    if (LeftoverBytes) {
      ...
      for (unsigned LoadSize = RegSize / 2, TotalSizeLoaded = 0;
        ...
        SDValue LoadPtr = DAG.getNode(ISD::ADD, SL, PtrTy, Arg,
                                      DAG.getConstant(Offset, PtrTy));
        SDValue LoadVal =
          DAG.getExtLoad(ISD::ZEXTLOAD, SL, RegTy, Chain, LoadPtr,
        ...
        SDValue Shift = DAG.getNode(ISD::SHL, SL, RegTy, LoadVal,
                                    DAG.getConstant(Shamt, MVT::i32));

        if (Val.getNode())
          Val = DAG.getNode(ISD::OR, SL, RegTy, Val, Shift);
        else
          Val = Shift;
  ...
  SDValue Src = DAG.getNode(ISD::ADD, SL, PtrTy, Arg,
                            DAG.getConstant(Offset, PtrTy));
  SDValue Dst = DAG.getNode(ISD::ADD, SL, PtrTy, StackPtr,
                            DAG.getIntPtrConstant(ByVal.Address));
  Chain = DAG.getMemcpy(Chain, SL, Dst, Src,
...
SDValue
RISCVTargetLowering::LowerFormalArguments(SDValue Chain,
  ...
  if (!OutChains.empty()) {
    ...
    /* unsigned Opcode, SDLoc DL, EVT VT, SDValue N, */
    
    EVT tVT(MVT::Other);
    //Chain = DAG.getNode(ISD::TokenFactor, SL, tVT,
...
static SDValue getTargetNode(SDValue Op, SelectionDAG &DAG, unsigned Flag) {
  ...
  if (GlobalAddressSDNode *N = dyn_cast<GlobalAddressSDNode>(Op))
    return DAG.getTargetGlobalAddress(N->getGlobal(), SL, Ty, 0,
                                      Flag);
...
static SDValue getAddrNonPIC(SDValue Op, SelectionDAG &DAG) {
  ...
  SDValue ResHi = DAG.getNode(RISCVISD::Hi, SL, Ty, Hi);
  SDValue ResLo = DAG.getNode(RISCVISD::Lo, SL, Ty, Lo);
  return DAG.getNode(ISD::ADD, SL, Ty, ResHi, ResLo);
}

static SDValue getAddrPIC(SDValue Op, SelectionDAG &DAG) {
  SDLoc SL( Op.getNode() );
  EVT Ty = Op.getValueType();
  return DAG.getNode(RISCVISD::PCREL_WRAPPER, SL, Ty, Op);
}
...
RISCVTargetLowering::LowerCall(CallLoweringInfo &CLI,
  ...
  Chain = DAG.getCALLSEQ_START(Chain, DAG.getConstant(NumBytes, PtrVT, true), SL);
  ...
  for (unsigned I = 0, E = ArgLocs.size(); I != E; ++I) {
    ...
    if (Flags.isByVal()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, SL, Subtarget.isRV64() ? RISCV::sp_64 : RISCV::sp, PtrVT);
      ...
      ArgValue = convertValVTToLocVT(DAG, SL, VA, ArgValue);
    ...
    if (VA.isRegLoc())
      ...
      SDValue Address = DAG.getNode(ISD::ADD, SL, PtrVT, StackPtr,
                                    DAG.getIntPtrConstant(Offset));

      // Emit the store.
      MemOpChains.push_back(DAG.getStore(Chain, SL, ArgValue, Address,
                                         MachinePointerInfo(),
                                         false, false, 0));
    }
  }
  ...
  for (unsigned I = 0, E = RegsToPass.size(); I != E; ++I) {
    Chain = DAG.getCopyToReg(Chain, SL, RegsToPass[I].first,
  ...
  if (GlobalAddressSDNode *G = dyn_cast<GlobalAddressSDNode>(Callee)) {
    if (TM.getRelocationModel() == Reloc::PIC_) {
      ...
      Callee = DAG.getTargetGlobalAddress(G->getGlobal(), SL, PtrVT);
  ...
  Chain = DAG.getNode(RISCVISD::CALL, SL, NodeTys, Ops[0] );
  Glue = Chain.getValue(1);
  ...
  for (unsigned I = 0, E = RetLocs.size(); I != E; ++I) {
    ...
    SDValue RetValue = DAG.getCopyFromReg(Chain, SL, VA.getLocReg(),
                                          VA.getLocVT(), Glue);
...
RISCVTargetLowering::LowerReturn(SDValue Chain,
  ...
  if (RetLocs.empty())
    return DAG.getNode(RISCVISD::RET_FLAG, SL, MVT::Other, Chain);
  ...
  for (unsigned I = 0, E = RetLocs.size(); I != E; ++I) {
    ...
    RetValue = convertValVTToLocVT(DAG, SL, VA, RetValue);

    // Chain and glue the copies together.
    unsigned Reg = VA.getLocReg();
    Chain = DAG.getCopyToReg(Chain, SL, Reg, RetValue, Glue);
    Glue = Chain.getValue(1);
  ...
  return DAG.getNode(RISCVISD::RET_FLAG, SL, MVT::Other,
		     RetOps[0], RetOps[1],
		     RetOps[2], RetOps[3] );
...
lowerSELECT_CC(SDValue Op, SelectionDAG &DAG) const
{
  ...
  SDValue Cond = DAG.getNode(ISD::SETCC, SL, getSetCCResultType(*DAG.getContext(), Ty),
                             Op.getOperand(0), Op.getOperand(1),
                             Op.getOperand(4));

  return DAG.getNode(ISD::SELECT, SL, Op.getValueType(), Cond, Op.getOperand(2),
                     Op.getOperand(3));
}
...
SDValue RISCVTargetLowering::lowerRETURNADDR(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getCopyFromReg(DAG.getEntryNode(), SL, Reg, VT);
}
...
SDValue RISCVTargetLowering::lowerGlobalAddress(SDValue Op,
  ...
  if (GlobalAddressSDNode *G = dyn_cast<GlobalAddressSDNode>(Op)) {
      SDLoc SL( Op.getNode() );
      Op = DAG.getTargetGlobalAddress(G->getGlobal(), SL, getPointerTy());
      return Op;
  }
...
SDValue RISCVTargetLowering::lowerGlobalTLSAddress(GlobalAddressSDNode *GA,
  ...
  if (model == TLSModel::LocalExec) {
    ...
    SDLoc SLTls( TlsGetAddr.getNode() );
    SDValue TGAHi = DAG.getTargetGlobalAddress(GV, SLTls, PtrVT, 0,
                                               RISCVII::MO_TPREL_HI);
    SDValue TGALo = DAG.getTargetGlobalAddress(GV, SLTls, PtrVT, 0,
                                               RISCVII::MO_TPREL_LO);
    SDLoc SLHi( TGAHi.getNode() );
    SDLoc SLLo( TGALo.getNode() );
    SDValue Hi = DAG.getNode(RISCVISD::Hi, SLHi, PtrVT, TGAHi);
    SDValue Lo = DAG.getNode(RISCVISD::Lo, SLLo, PtrVT, TGALo);
    Offset = DAG.getNode(ISD::ADD, SLTls, PtrVT, Hi, Lo); 
  } else {
  ...
  SDLoc SLtp( ThreadPointer.getNode() ); 
  return DAG.getNode(ISD::ADD, SLtp, PtrVT, ThreadPointer, Offset);
...
SDValue RISCVTargetLowering::lowerJumpTable(JumpTableSDNode *JT,
  ...
  SDLoc SL( Result.getNode() );

  // Use LARL to load the address of the table.
  return DAG.getNode(RISCVISD::PCREL_WRAPPER, SL, PtrVT, Result);
}
...
SDValue RISCVTargetLowering::lowerVASTART(SDValue Op,
  ...
  return DAG.getStore(Chain, SL, FI, Addr,
...
SDValue RISCVTargetLowering::lowerVAARG(SDValue Op, SelectionDAG &DAG) const{
  ...
  SDValue VAList = DAG.getLoad(PtrVT, SL, InChain, VAListPtr,
                               MachinePointerInfo(SV), false, false, false, 0);
  // Increment the pointer, VAList, to the next vaarg.
  SDValue NextPtr = DAG.getNode(ISD::ADD, SL, PtrVT, VAList,
                                DAG.getIntPtrConstant(VT.getSizeInBits()/8));
  // Store the incremented VAList to the legalized pointer.
  InChain = DAG.getStore(VAList.getValue(1), SL, NextPtr,
                         VAListPtr, MachinePointerInfo(SV), false, false, 0);
  // Load the actual argument out of the pointer VAList.
  // We can't count on greater alignment than the word size.
  return DAG.getLoad(VT, SL, InChain, VAList, MachinePointerInfo(),
                     false, false, false,
...
SDValue RISCVTargetLowering::lowerATOMIC_FENCE(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(RISCVISD::FENCE, SL, MVT::Other,Op.getOperand(0),
...
SDValue RISCVTargetLowering::lowerSTACKSAVE(SDValue Op,
  ...
  SDLoc SL( Op.getNode() );
  return DAG.getCopyFromReg(Op.getOperand(0), SL,
                            sp, Op.getValueType());
}
...
SDValue RISCVTargetLowering::lowerSTACKRESTORE(SDValue Op,
  ...
  unsigned sp = Subtarget.isRV64() ? RISCV::sp_64 : RISCV::sp;
  SDLoc SL( Op.getNode() );
  return DAG.getCopyToReg(Op.getOperand(0), SL, 
                          sp, Op.getOperand(1));
}
...
lowerFRAMEADDR(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDLoc SL( Op.getNode() );
  SDValue FrameAddr = DAG.getCopyFromReg(DAG.getEntryNode(), SL,
...
RISCVTargetLowering::writeVarArgRegs(std::vector<SDValue> &OutChains,
  ...
  for (unsigned I = Idx; I < NumRegs; ++I, VaArgOffset += RegSize) {
    ...
    SDValue ArgValue = DAG.getCopyFromReg(Chain, SL, Reg, RegTy);
    SDLoc SLArg( ArgValue.getNode() );
    FI = MFI->CreateFixedObject(RegSize, VaArgOffset, true);
    SDValue PtrOff = DAG.getFrameIndex(FI, getPointerTy());
    SDValue Store = DAG.getStore(Chain, SLArg, ArgValue, PtrOff,
                                 MachinePointerInfo(), false, false, 0);
    cast<StoreSDNode>(Store.getNode())->getMemOperand()->setValue((const Value *)(0));
    OutChains.push_back(Store);
  }
}
